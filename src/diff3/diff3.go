...
import (
	...
	"github.com/costaluu/flag/diff3/linereader"
)
...
func Merge(yours, base, theirs io.Reader, detailed bool, yoursLabel string, theirsLabel string) (*MergeResult, error) {
	al, err := linereader.GetLines(yours)
	if err != nil {
		return nil, err
	}
	ol, err := linereader.GetLines(base)
	if err != nil {
		return nil, err
	}
	bl, err := linereader.GetLines(theirs)
	if err != nil {
		return nil, err
	}
			...
			if detailed {
				...
				for j := 0; j < len(c); j++ {
					...
					if inner.common != nil {
						...
						conflicts = true
						lines = addConflictMarkers(lines, inner.file1, inner.file2, yoursLabel, theirsLabel)
					}
				}
			} else {
				conflicts = true
				lines = addConflictMarkers(lines, item.conflict.a, item.conflict.b, yoursLabel, theirsLabel)
			}
		}
	}
